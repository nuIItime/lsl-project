...
{ 
list a = llGetObjectDetails(llList2String(root_sitting(avatar),0),([OBJECT_PHYSICS]));
...
string p_zone(string position,key ID)
{
  ...
  if (!Length){ return position_A+" | "+avatar_parcel(ovF)+" | "+A_status(ID); }else
  {
  integer x;
  for ( ; x < Length; x += 1)
  {
...
} } } return position_A+" | "+avatar_parcel(ovF)+" | "+A_status(ID); }
...
visit_logs_send(string A,integer B) 
{
    ...
    "Parcel : "+avatar_parcel((vector)pos);
    }
    if(B == 2)
    {
    string pos = llList2String(items,1);    
